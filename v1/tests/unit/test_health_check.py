...
class TestHealthCheckAggregation:
    ...
    async def test_services_registered(self, mock_settings):
        from v1.src.services.health_check import HealthCheckService, HealthStatus
        svc = HealthCheckService(mock_settings)
        await svc.initialize()
...
class TestHealthStatus:
    def test_enum_values(self):
        from v1.src.services.health_check import HealthStatus
...
class TestHealthCheck:
    def test_health_check_dataclass(self):
        from v1.src.services.health_check import HealthCheck, HealthStatus
